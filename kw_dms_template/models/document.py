...
class Document(models.Model):
        ...
        for obj in self:
            if obj.contact_id:
                template = self.env['sms.template']._render_template(
                    ...
                    model=obj.model_name, res_ids=[obj.contact_id.id])
                obj.body = template[obj.contact_id.id]
    ...
    def prepare_html(self, html):
        ...
        layout = self.env.ref(
            'kw_dms_template.kw_document_template_minimal_layout', False)
        if not layout:
            return {}
    ...
    def create(self, vals_list):
        ...
        if obj.type_id and obj.is_generated and obj.contact_id:
            try:
                obj.name = f'{obj.type_id.name}, {obj.contact_id.name}'
                obj.body = obj.render_message(
                    obj.type_id.template_id.body_html,
                    obj.model_name, obj.contact_id.id)[obj.contact_id.id]
