        ...
        for obj in self:
            if obj.sale_order_id:
                template = self.env['sms.template']._render_template(
                    ...
                    model=obj.model_name, res_ids=[obj.sale_order_id.id])
                obj.body = template[obj.sale_order_id.id]

    @api.model
    def create(self, vals_list):
        obj = super().create(vals_list)
        if obj.type_id and obj.is_generated and obj.sale_order_id:
            try:
                obj.name = f'{obj.type_id.name}, {obj.sale_order_id.name}'
                obj.body = obj.render_message(
                    obj.type_id.template_id.body_html,
                    obj.model_name, obj.sale_order_id.id)[obj.sale_order_id.id]
